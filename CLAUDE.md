...
- [x] **Аудит калькуляторов перед продажами** — найдены и исправлены блокеры ТОО-калькулятора (НДС 12→16%, СН 9.5→6%, дивиденды завышали итог, ЕСП в оптимизаторе), коммит `05b3f3c`
- [ ] **Запустить `flutter analyze` и `flutter test`** локально — Device Guard блокирует dart.exe на этой машине. Переписаны тесты ЭСФ; правки калькуляторов и ЭСФ проверены чтением + node/python харнесс, но нужен прогон Dart
- [ ] **Уточнить у Фариды 3 вопроса**: (1) отменён ли режим ЕСП с 2026 — сейчас убран из оптимизатора, но справка осталась; (2) ВОСМС «за себя» — 5% от 1.4 МЗП по НК 2026?; (3) форма ИПН на ОУР — всё ещё 220.00?
...
- [ ] Форма 910 XML (после получения XSD из 1С) — `form910_xml_service.dart`
- [ ] Форма 200.00 XML (ИПН+СН за сотрудников)
- [ ] Форма 300.00 XML (НДС) — для ОУР
