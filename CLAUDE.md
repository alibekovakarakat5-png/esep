...
> 🆕 **Текущее состояние и на чём остановились — см. [HANDOFF.md](HANDOFF.md)** (сводка последней сессии: баг выписки, рассылка + авто-прогрев Connect, маркетинг-видео, гранты, заявка на Glovo).
...
| `469bba3` | fix(ui): баннер триала рендерился вертикально по одной букве (корень — `minimumSize: infinity` в теме) |
...
| ветка `claude/accountant-clients-server` | feat(accountant): клиенты бухгалтера хранятся на сервере — таблица `accounting_clients` (сотрудники и чеклист в JSONB), `GET/POST/PUT/DELETE /api/accounting/clients` (snake_case-контракт, camelCase тоже принимается, чужой id → 409), провайдер грузит с сервера с гейтом по авторизации, демо-клиенты только в демо-режиме. Было: `AccountingNotifier() : super(_demoClients())` — клиенты жили в памяти и исчезали при F5. Тесты: server 5, Dart 5; изоляция между бухгалтерами проверена curl-ом (чужие GET/PUT/DELETE не видят данных) |
| ветка `claude/qa-buhfirmy-kurerki` | fix(nav): бухгалтер не попадал в свой кабинет — две причины: `_routerListenableProvider` был `ChangeNotifierProvider` (ре-эмит на каждый notify → `appRouterProvider` пересобирал GoRouter → навигация падала на `initialLocation '/dashboard'`) и `auth_screen` жёстко звал `context.go('/dashboard')`. Тест `router_stability_test.dart`. Плюс: свой текст гайда для бухгалтера в «Расчётах» (было «не переплатите бухгалтеру 30-50 тыс» — самому бухгалтеру), онбординг и тур переписаны с «записывайте доходы» на импорт выписки, русский экран 404 вместо `GoException`. ⚠ Найдено и НЕ исправлено: (1) модуль бухгалтера на демо-данных в памяти (`AccountingNotifier() : super(_demoClients())`) — клиенты не сохраняются нигде; (2) ИПН в ФОТ считается без вычета ВОСМС (3 места, включая форму 200.00 в КГД) — вопрос Фариде; (3) кабинет Platform без ключа = тупик «нет доступа»; (4) дедлайн 15.08.2026 суббота — перенос на понедельник не показывается |
| ветка `claude/fix-transactions-contract` (4) | feat(import): умный предвыбор галочек при импорте PDF (`ImportAutoSelect`: снимает покупки, переводы <5000 ₸, свои пополнения/депозит/зарплату — на выписке Каракат 1429 → 257 к проверке), память решений «бизнес/личное» (`SelectionMemory`, Hive), чипы быстрых действий, карточка-инструкция при первом импорте; bulk-импорт одним POST (было 2 запроса на строку). ⚠ Найдено: free-тариф = 10 операций/мес и 7-дневный триал лимиты НЕ расширяет — импорт выписки на триале упрётся в 403 (вопрос Каракат, не менял) |
| ветка `claude/fix-transactions-contract` (3) | feat(import): PDF-выписка Kaspi Gold — разбор на сервере (`pdf-parse`, `POST /api/import/kaspi-pdf`, `services/kaspi_pdf.js`; регэксп строк «DD.MM.YY ± сумма ₸ Тип Детали»), клиент шлёт .pdf туда и мапит в KaspiParseResult (`fromServerPdf`); на реальной выписке 34 стр → 1429 операций. Плюс fix(ui): themeMode.light принудительно — в системной тёмной теме текст пропадал (найдено Каракат) |
| ветка `claude/fix-transactions-contract` (2) | fix(auth): режим ИП/ТОО/Бухгалтер хранится в профиле (`users.user_mode`, `PATCH /api/auth/mode`; login/me отдают `userMode`, Flutter применяет через `applyRemote`) — «Кто вы?» спрашивается один раз, повторный вход и новое устройство идут сразу в кабинет. Найдено Каракат при тесте под test@gmail.com |
| `b73e101` | fix(server): контракт transactions выровнен под Dart-модель — GET/POST/PUT принимают и отдают snake_case (`is_income`/`client_name`, legacy camelCase тоже принимается, по образцу invoices.js); фикс сдвига даты на −1 день (`toISOString` на локальной полуночи); юнит `transactions_contract.test.js` + серверные тесты добавлены в `branch-tests.yml`. До фикса: операции «исчезали» после перезагрузки (GET не парсился), ручное добавление и импорт выписки получали 400 — вероятная причина «14 юзеров, 0 операций». Мердж в main = автодеплой Railway |
...
- [x] **Запустить `flutter test` локально** — РЕШЕНО 2026-08-12: Device Guard не действует на Linux-контейнеры, тесты гоняются в Docker: `docker run --rm -v "<repo>:/work" -v esep-pub-cache:/root/.pub-cache -w /work ghcr.io/cirruslabs/flutter:stable bash -lc "flutter test"` (образ спуллен; после прогона откатывать pubspec.lock — stable-канал двигает версии). Плюс CI `branch-tests.yml` гоняет тесты на любой ветке без деплоя. `flutter analyze` так же при надобности
...
### 🎨 Интерфейс (после диагноза «14 юзеров, 0 операций»)

- [x] Шрифт Inter вместо системного — `eb962a6`
- [x] Экран первого запуска `/start` «Начало работы» — `9da629b`
- [ ] **Убрать `minimumSize: Size(double.infinity, 52)` из темы** и обернуть
      9 зависящих от неё кнопок в `SizedBox(width: double.infinity)`.
      Это ловушка: любой `ElevatedButton` внутри `Row` ломает соседний
      `Expanded` (так сломался `TrialBanner`). Делать вместе с редизайном —
      нужно визуально проверить каждый экран
...
|---|---|---|---|
| ~~Webkassa Code 4~~ | «Нет прав к функционалу Курьеры» | ✅ СНЯТ — загрузка заказов работает. Для UI-режима в ЛК: право «Управление режимом курьера» (Программирование → Упр. организацией → Упр. кассирами → Права) + тумблер «Режим „Курьер"» на кассе (Упр. кассами → Редактирование → Специальные режимы). Просмотр: Заказы (Курьеры) → Управление заказами / Чеки по заказам | сделано |
| **Договор с КГД (ИСНА API)** | Для сервисов #8 (реестр), #9 (льготы) | ✉ **Письмо отправлено 2026-08-06** (Каракат, в КГД и ИСНА). Ждём ответ; при получении доступа — заменить заглушки registry/benefits на живые вызовы | ответ ~2-4 недели |
| **Webhook URL в Webkassa** | Куда Webkassa шлёт уведомление о фискализации | Самостоятельного внесения НЕТ (ответ поддержки 2026-08-06): URL отправляется ответным письмом, вносят они. Для боевой среды — URL + ИИН/БИН организации, зарегистрированной в боевой среде. URL: `https://api.esepkz.com/api/platform/webhooks/webkassa-courier` — прод-роут жив (проверен: 400 invalid payload на фейк) | письмо готово |
| **Боевой api-key Webkassa** | Сейчас только тестовая среда devkkm | Совместное тестирование с Webkassa (ответ поддержки 2026-08-06); для связи просят Telegram-ник | после письма |
...
- [x] Письмо в КГД/ИСНА на доступ к API — **отправлено 2026-08-06** (для сервисов #8, #9); ждём ответ
