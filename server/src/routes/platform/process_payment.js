...
router.post(
  ...
  async (req, res) => {
    ...
    if (afterPayment > MONTHLY_LIMIT_TENGE) {
      ...
      result.next_actions = [
        {
          code: 'defer_next_month',
          title: 'Перенести остаток на следующий месяц',
          detail: `Лимит месячный, обнуляется 1 числа. Остаток ${excess} ₸ можно выплатить ` +
                  `в следующем месяце как самозанятому.`,
        },
        {
          code: 'register_ip',
          title: 'Оформить водителю ИП',
          detail: `Если водитель стабильно зарабатывает больше лимита — оформите ИП (онлайн, ~1 день). ` +
                  `Дальше выплаты продолжатся через Esep уже по режиму ИП, без лимита 300 МРП.`,
        },
      ];
      result.processed_in_ms = Date.now() - startedAt;
