  ...
  for (int offset = -1; offset <= (days ~/ 28) + 2; offset++) {
    ...
    switch (client.regime) {
      ...
      case ClientTaxRegime.our:
        ...
        // 100.00 — годовая декларация по КПН. Сдают только юрлица (ТОО) на ОУР;
        // ИП на ОУР платит ИПН (форма 220.00), а не КПН, поэтому 100.00 ему не
        // ставим. Срок сдачи — 31 марта года, следующего за отчётным
        // (ст. 306/315 НК-2026); уплата КПН — до 10 апреля. Дата и подпись «за
        // $year» синхронизированы с 700, чтобы на одной дате не было расхождений.
        if (client.entityType == ClientEntityType.too) {
          add('100', 'Форма 100.00 (КПН) за $year', DateTime(year, 3, 31));
        }

        // 700.00 — налог на имущество/транспорт/землю (НЕ КПН, КПН это форма
        // 100). Срок — 31 марта года, следующего за отчётным.
        add('700', 'Форма 700.00 (имущество/транспорт/земля) за $year',
            DateTime(year, 3, 31));
    }
  }
